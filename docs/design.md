...
Arrays and `inet` are **query-runtime only** and cannot be stored in columns.

Implications:

- Model properties mapped to arrays, ranges, `hstore`, `tsvector`, geometry, enums or custom
  composite types must be rejected by the model validator with a clear message.
